...
describe('Vault', () => {
    ...
    describe('exists', () => {
        it('returns a promise with expected exists value', () => {
            ...
            mocked(oliveHelps.vault.exists).mockImplementation((key, callback) => callback(expectedResult));
    ...
    describe('write', () => {
        it('returns a promise with expected write value', () => {
            ...
            expect(oliveHelps.vault.write).toHaveBeenCalledWith(expectedKey, expectedValue, expect.any(Function));
            return expect(actual).resolves.toBeUndefined();
        });
