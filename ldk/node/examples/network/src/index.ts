...
class ExampleLoop implements Loop {
  ...
  start(host: HostServices): void {
    this._host = host;
    const now = moment();
    const url = `https://api.fda.gov/food/enforcement.json?search=report_date:[${now.subtract(3, 'months').startOf('month').format('YYYYMMDD')}+TO+${now.endOf("month").format('YYYYMMDD')}]&limit=1`;
